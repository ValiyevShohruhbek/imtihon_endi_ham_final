  ...
  return (
    <header className="con h-[56px] w-full flex items-center justify-between">
      <div className="header-left flex items-center gap-2">
        <button onClick={toggle}><HiMiniBars3 /></button>
        <Link to="/" className="text-[black] flex items-center gap-1">
          <Icon.youTubeIcon /> <span className="font-bold font-poppins">YouTube</span>
        </Link>
      </div>

      <div className="header-center flex items-center gap-4">
        <div className="flex items-center overflow-hidden ">
          <input type="text" placeholder="Search" className="w-[540px] placeholder:text-[20px] outline-[#065FD4] border-[#d3d3d3] h-[35px] border p-[10px_4px_11px_17px] rounded-[40px_0px_0px_40px]" />
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleClick} className={data ? `hidden` : `block`}>
            <div className="w-[102px] h-[36px] rounded-[20px] border border-[#d3d3d3] flex items-center justify-center gap-1.5">
              ...
              <span className="text-[14px] text-[#065FD4] font-bold">Sign In</span>
            </div>
          </button>

          <button className={data ? ` flex items-center gap-1 text-[17px] border rounded-md px-2  hover:bg-[#cfcfcf61] ` : `hidden`}>
          ...
          <div className={data ? `info border w-[40px] h-[36px] bg-[purple] text-white rounded-full text-[20px] flex items-center justify-center` : "hidden"}>
